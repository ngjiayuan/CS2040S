...
public class GameTree {
    ...
    int findValue() {
        ...
        if (root == null) {
            ...
            // assumes PLAYER A always starts
            // if PLAYER B starts, counter should be odd
            return getValue(root, 0);
        }
    }

    /**
     * returns value of any node after recursion of all its children
     * @param node any TreeNode
     * @param counter increments each time it goes down a level of the tree
     *                signifies change of player and hence change of max or min
     * @return value of node
     */
    int getValue(TreeNode node, int counter) {
        if (node.leaf) {
            return node.value;
        } else if (counter%2 == 0) { // if PLAYER A, find MAX
            int[] maxArray = new int[node.numChildren];
            int i = 0;
            for (TreeNode child : node.children) {
                maxArray[i] = getValue(child, counter + 1);
                i++;
            }
            node.value = maxValue(maxArray);
        } else { // if PLAYER B, find MIN
            int[] minArray = new int[node.numChildren];
            int i = 0;
            for (TreeNode child : node.children) {
                minArray[i] = getValue(child, counter + 1);
                i++;
            }
            node.value = minValue(minArray);
        }
        return node.value;
    }

    // returns min value of an integer array
